...
async function all_signalisations() {
    try {
      const response = await axios.get('http://127.0.0.1:8000/api/auth/signalisationTrash')
...
function delete_signalisation(id)
{
    axios.delete('http://127.0.0.1:8000/api/auth/signalisation/' + id)
    .then(function (response) {
      // setter
    ...
    axios.delete('http://127.0.0.1:8000/api/auth/deleteAllSignales/' +id)
    .then(function (response) {
      // setter
      //const token = localStorage.setItem('token', response.data.access_token)
      //const user = localStorage.setItem('user', response.data.user)
...
<td><button type="button" className="btn btn-sm btn-danger" data-toggle="modal" data-target={'#_' + element['id']}>Supprimée</button>


<div className="modal fade" id={'_' + element['id']} tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
      <div className="modal-dialog modal-dialog-centered" role="document">
        <div className="modal-content">
          <div className="modal-header">
          ...
          <div className="modal-body">
            Êtes-vous sûr ? Cette sinialisation sera supprimée et vous ne pourrez plus la voir!
          </div>
          <div className="modal-footer">
            <button type="submit" className="btn btn-danger" onClick={() => {delete_signalisation(element['id'])}}>Supprimée</button>
            <button type="button" className="btn btn-outline-info" data-dismiss="modal">Fermer</button>
          </div>
        </div>
      </div>
